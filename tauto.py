...
parser.add_argument("--identifier", type=str, default="torgoLM", help="Identifier")
parser.add_argument("--arpa_dir", type=str, default="torgo_lm_arpa_files", help="Directory for ARPA files")
...
if ngram_order == 1:
  ...
  kenlm_model = f"{test_speaker}_3gram_{pattern}.arpa"
...
def evaluateModel(processor, model, dataset, lm_model_path=None):
  ...
  if not lm_model_path:
    ...
    with open(f"{lm_local_path}/{test_speaker}_unigram_{pattern}.txt", "r") as f:
      for line in f:
        line = line.strip()
        unigrams.add(line)
...
with open(f"results_{identifier}_{test_speaker}_{test_speaker}_{level}_{pattern}_{split}.txt", "w") as csv_file:
